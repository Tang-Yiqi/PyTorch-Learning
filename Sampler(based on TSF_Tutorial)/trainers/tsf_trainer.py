...
class Trainer(base_trainer):
    ...
    def train(self):
        ...
        # 确定使用的数据类型
        dtype = torch.float32
        if hasattr(self.configs.sync, 'dtype'):
            if self.configs.sync.dtype == 'float32':
                dtype = torch.float32
            elif self.configs.sync.dtype == 'float64':
                dtype = torch.float64
        ...
        for epoch in range(self.configs.epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(self.train_loader)):
                # 将输入数据移至指定设备并设置为指定数据类型
                batch_x = batch_x.to(dtype=dtype).to(self.device)
                batch_y = batch_y.to(dtype=dtype).to(self.device)
            ...
            wandb.log({
                "MAE": vali_metrics['mae'],
                "MSE": vali_metrics['mse'],
                "MAPE": vali_metrics['mape']
            })
