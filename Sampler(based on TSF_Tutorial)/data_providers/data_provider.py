...
class CustomDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        # 确定使用的数据类型
        dtype = torch.float32
        if hasattr(self.args.sync, 'dtype'):
            if self.args.sync.dtype == 'float32':
                dtype = torch.float32
            elif self.args.sync.dtype == 'float64':
                dtype = torch.float64
        
        # 将数据转换为张量，并指定数据类型和设备
        data = torch.tensor(data[border1:border2], dtype=dtype, device=self.args.sync.device)
        # 保存原始数据
        self.data = data
        # 保存时间戳数据
    ...
    def __getitem__(self, index):
        ...
        # 获取输入数据，形状为[通道数, 历史长度]
        x = self.data[index - self.his_len:index, ...].transpose(-1, -2).float()
        # 获取目标数据，形状为[通道数, 预测长度]
        y = self.data[index:index + self.pred_len, ...].transpose(-1, -2).float()
        # 设置输入数据的时间标记
    ...
    def __len__(self):
        ...
        return self.data.shape[0] - self.his_len - self.pred_len + 1
    ...
    def transform(self, data):
        ...
        # 使用均值和标准差进行标准化，确保使用相同的数据类型
        means = torch.tensor(self.means, dtype=dtype, device=device)
        stds = torch.tensor(self.stds, dtype=dtype, device=device)
